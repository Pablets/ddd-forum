...
export class UpdatePostStats implements UseCase<UpdatePostStatsDTO, Promise<Response>> {
  ...
  public async execute(response: UpdatePostStatsDTO): Promise<Response> {
    ...
    try {
      ...
      const commentCount: number = await this.postRepo.getNumberOfCommentsByPostId(response.postId);

      // Update comment count
      post.updateTotalNumberComments(commentCount);

      // Update post points
      const [numPostUpvotes, numPostDownvotes, commentUpvotes, commentDownvotes] =
        await Promise.all([
